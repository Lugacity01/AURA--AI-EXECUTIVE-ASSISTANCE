...
export default function ApprovalsHub() {
  ...
  const handleBulkApprove = () => {
    ...
    const draftIds = selectedIds
      .map(id => items.find(i => i.id === id)?.draftId)
    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-6 w-full">
      <div className="flex flex-col gap-6 text-left select-none max-w-5xl mx-auto w-full">
        ...
        {isLoading ? (
          ...
          <div className="p-16 text-center border border-dashed border-white/[0.06] rounded-2xl flex flex-col items-center justify-center bg-white/[0.01]">
            <FolderOpen className="w-12 h-12 text-slate-600 mb-4 animate-pulse" />
            <h4 className="text-sm font-semibold text-slate-300">Approvals Queue Clear</h4>
            <p className="text-xs text-slate-500 mt-1 max-w-sm text-center leading-relaxed">
              No items require your verification. Aura is managing outbound correspondence according to your safety guidelines.
            </p>
          </div>
        ) : (
